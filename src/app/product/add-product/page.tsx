...
export default function AddProduct() {
    ...
    ///const category =  useSelector((state : RootState) => state.Admin.category) as CategoryData[] | undefined
	 ...
	 const [category, setSategory] = useState<any[]>([]);
  
   useEffect(() => {
    axios.get("/api/common/category/getCategory").then((res) => setSategory(res.data.data));
  }, []);
    ...
    const onSubmit: SubmitHandler<Inputs> = async data => {
		...
		const uploadImageToFirebase = 'product.jpeg';
